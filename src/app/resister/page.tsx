...
const RegisterPage = () => {
  const onSubmit =async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const {name, email,image, password} = Object.fromEntries(formData.entries()) as {name: string, email: string, image: string, password: string};
    formData.forEach((value, key) => {
      data[key] = value.toString();
    });
    const { data:submitData, error } = await authClient.signUp.email({
    name, // required
    email, // required
    password, // required
    image,
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 shadow-xl sm:p-8 transition-colors duration-200">
        ...
        <Form className="flex flex-col gap-5" onSubmit={onSubmit}>
          ...
          <TextField isRequired name="name" type="text" className="w-full">
            ...
            <Input placeholder="John Doe" className="mt-1.5 text-white" />
          ...
          <TextField isRequired name="image" type="text" className="w-full">
            ...
            <Input placeholder="https://example.com/image.jpg" className="mt-1.5 text-white" />
            <FieldError className="text-xs text-danger mt-1" />
          </TextField>
